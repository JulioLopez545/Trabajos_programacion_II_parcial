...
class ActualizarProveedor(View):
    def get(self,request, uuid):
        ...
        form = UpdateProveedorFrom(instance=proveedor)

        return render(request, 'proveedores/agregar_proveedor.html', {'form':form})
        
    def post(self, request, uuid):
            obj = Proveedor.objects.get(uuid=uuid)
            form= UpdateProveedorFrom(request.POST, instance=obj)
            if(form.is_valid()):
...
class   ActualizarProductos(View):
    def get(self,request, uuid):
        producto= Producto.objects.get(uuid=uuid)
        form = UpdateProductForm(instance=producto)

        return render(request, 'productos/agregar_producto.html', {'form':form})
        
    def post(self, request, uuid):
            obj = Producto.objects.get(uuid=uuid)
            form= UpdateProductForm(request.POST, instance=obj)
            if(form.is_valid()):
                obj = form.save(commit=False)
                obj.save()
                return HttpResponseRedirect(reverse('listar_productos'))
            ...
            return render(request, 'productos/ActualizarProductos.html', {'form': form})
